...
void ion_mm_heap_free(struct ion_buffer *buffer)
{
    ...
    for_each_sg(table->sgl, sg, table->nents, i) {
        if (sg->offset != 0 || sg->length == 0 || sg->length % PAGE_SIZE != 0)
            IONMSG("%s: Waring sg table maybe have problem, i = %d, sg->offset = 0x%x, sg->length = 0x%x, address = 0x%llx.\n", 
                    __func__, i, sg->offset, sg->length, (unsigned long long)sg->dma_address);
    }
    ...
    for_each_sg(table->sgl, sg, table->nents, i) {
                if (sg->offset != 0 || sg->length == 0 || sg->length % PAGE_SIZE != 0) {
                    IONMSG("%s: Waring sg table maybe have problem, don't free, i = %d, offset = 0x%x, length = 0x%x, address = 0x%llx.\n", 
                            __func__, i, sg->offset, sg->length, (unsigned long long)sg->dma_address);
                    ion_aee_print("ion_mm_heap_free sg table error!.\n");
                    continue;
                }
