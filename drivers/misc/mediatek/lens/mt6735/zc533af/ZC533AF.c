...
#define AF_DRVNAME "ZC533AF"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x20
#define PLATFORM_DRIVER_NAME "lens_actuator_zc533af"
#define AF_DRIVER_CLASS_NAME "actuatordrv_zc533af"
...
static int s4AF_ReadReg(u8 addr, u8 *a_pu2Result)
{
    int  i4RetValue = 0;
    char pBuff[2];
...
static int s4AF_WriteReg(u8 addr, u8 a_u2Data)
{
    int  i4RetValue = 0;

    char puSendCmd[2] = {addr, a_u2Data};
...
static int s4AF_ReadPos(u16 *a_pu2Result)
{
	int  i4RetValue = 0;
	u8 pos_hi, pos_lo;

	i4RetValue |= s4AF_ReadReg(0x03, &pos_hi);
	i4RetValue |= s4AF_ReadReg(0x04, &pos_lo);
...
static int s4AF_WritePos(u16 a_u2Data)
{
    int  i4RetValue = 0;

    i4RetValue = s4AF_WriteReg(0x03, ((a_u2Data>>8) & 0x03));
    i4RetValue |= s4AF_WriteReg(0x04, a_u2Data&0xff);

    return 0;
}

inline static int getAFInfo(__user stZC533AF_MotorInfo * pstMotorInfo)
{
    stZC533AF_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stZC533AF_MotorInfo)))
    {
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stZC533AF_MotorMETAInfo * pstMotorMETAInfo)
{
    stZC533AF_MotorMETAInfo stMotorMETAInfo;
    ...
    if(copy_to_user(pstMotorMETAInfo , &stMotorMETAInfo , sizeof(stZC533AF_MotorMETAInfo)))
    {
        LOG_INF("copy to user failed when getting motor information \n");
    }
...
inline static int moveAF(unsigned long a_u4Position)
{
    ...
    //LOG_INF("move [curr] %d [target] %d\n", g_u4CurrPosition, g_u4TargetPosition);
            ...
            if(s4AF_WritePos((unsigned short)g_u4TargetPosition) == 0)
            {
                spin_lock(&g_AF_SpinLock);
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case ZC533AFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stZC533AF_MotorInfo *)(a_u4Param));
        break;
        #ifdef LensdrvCM3
        case ZC533AFIOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stZC533AF_MotorMETAInfo *)(a_u4Param));
        break;
        #endif
        case ZC533AFIOC_T_MOVETO :
            i4RetValue = moveAF(a_u4Param);
        break;

        case ZC533AFIOC_T_SETINFPOS :
            i4RetValue = setAFInf(a_u4Param);
        break;

        case ZC533AFIOC_T_SETMACROPOS :
...
static int AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    if(g_s4AF_Opened)
    {
		...
		s4AF_WriteReg(0x02, 0x00); //poweron
    spin_lock(&g_AF_SpinLock);
    g_s4AF_Opened = 1;
    spin_unlock(&g_AF_SpinLock);
    
    s4AF_ReadReg(0x00, &id1);
    s4AF_ReadReg(0x01, &id2);
    printk("id1=0x%x, id2=0x%x\n", id1, id2);
...
//        s4AF_WriteReg(200);
 ...
 //       s4AF_WriteReg(100);
    ...
    s4AF_WriteReg(0x02, 0x01); //poweroff
...
static int __init ZC533AF_i2C_init(void)
{
...
static void __exit ZC533AF_i2C_exit(void)
{
    platform_driver_unregister(&g_stAF_Driver);
}

module_init(ZC533AF_i2C_init);
module_exit(ZC533AF_i2C_exit);

MODULE_DESCRIPTION("ZC533AF lens module driver");
