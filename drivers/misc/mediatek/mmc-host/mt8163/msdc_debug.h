...
//#define ___MSDC_DEBUG_IN_MTKSTYLE___
...
#ifndef ___MSDC_DEBUG_IN_MTKSTYLE___
#define MSDC_DBG_PRINT(cond, fmt, ...)   \
   ((cond) ? (printk(KERN_ERR "%s:"WHEREDOESMSDCRUN, MSDC_FILTER_NAME(cond)),\
   				       printk(KERN_ERR fmt" <- L<%d> PID<%s><0x%x>\n", \
...
#else  /* ___MSDC_DEBUG_IN_MTKSTYLE___*/

#define MSDC_DBG_PRINT(cond, fmt, ...)   \
   ((void)((cond) ? (xlog_printk(ANDROID_LOG_ERROR, "%s:"WHEREDOESMSDCRUN, MSDC_FILTER_NAME(cond)),\
   				     xlog_printk(ANDROID_LOG_ERROR, fmt" <- L<%d> PID<%s><0x%x>\n", \
   				     			##__VA_ARGS__, __LINE__, current->comm, current->pid),1):0))
   				  
#define MSDC_DBGCHK(friendname, exp) \
   ((void)((exp)?( xlog_printk(ANDROID_LOG_ERROR, friendname": DBGCHK failed in file %s at line %d \r\n", \
                 												  __FILE__ ,__LINE__ ),  \
	   																  		  			  1)  \
   :0))
#endif  /* ___MSDC_DEBUG_IN_MTKSTYLE___*/
...
#define MSDC_ASSERT(exp) \
   ((exp)?1:(          \
       printk(KERN_ERR ": MSDC_ASSERT MUSTBE: failed in file %s at line %d \r\n", \
                 __FILE__ ,__LINE__ ),    \
       MUSTBE(), \
       0  \
   ))
...
#define MSDC_ERR_PRINT(cond, fmt, ...)	 \
   ((cond)?(printk(KERN_ERR "***ERROR:%s:"WHEREDOESMSDCRUN": %s line %d: ", MSDC_FILTER_NAME(cond),__FILE__,__LINE__),\
   			printk(KERN_ERR fmt" <- PID<%s><0x%x>\n", ##__VA_ARGS__, current->comm, current->pid), 1) : 0)
...
#define MSDC_ASSERT(exp) \
   ((void)((exp)?1:(          \
       xlog_printk(ANDROID_LOG_ERROR, WHEREDOESMSDCRUN": MUSTBE: failed in file %s at line %d \r\n", \
                 __FILE__ ,__LINE__ ),    \
       MUSTBE(), \
       0  \
   )))
...
#define MSDC_ERR_PRINT(cond, fmt, ...)	 \
   ((void)((cond)?(xlog_printk(ANDROID_LOG_ERROR, "***ERROR:%s:"WHEREDOESMSDCRUN": %s line %d: ", MSDC_FILTER_NAME(cond),__FILE__,__LINE__),\
   			xlog_printk(ANDROID_LOG_ERROR, fmt" <- PID<%s><0x%x>\n", ##__VA_ARGS__, current->comm, current->pid),1):0))

#define MSDC_RAW_PRINT(cond, fmt, ...)   \
   ((void)((cond)?(xlog_printk(ANDROID_LOG_ERROR, fmt,##__VA_ARGS__),1):0))
