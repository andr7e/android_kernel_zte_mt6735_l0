...
static void simp_msdc_config_clock(struct simp_msdc_host *host, unsigned int hz)  /* no ddr */
{
    ...
    sdr_set_field(MSDC_CFG, MSDC_CFG_CKDIV,(div % 0xFFF));
...
#ifdef FPGA_PLATFORM
extern bool hwPowerOn_fpga(int id);
extern bool hwPowerSwitch_fpga(int id, int lowlevel);
extern bool hwPowerDown_fpga(int id);
#else
...
#ifdef FPGA_PLATFORM
    if(on)
        hwPowerOn_fpga(host->mtk_host->id);
    else
        hwPowerDown_fpga(host->mtk_host->id);
#else
#if 1
