...
phys_addr_t mtkfb_get_fb_base(void)
{
	...
	if (fb_base == 0) fb_base = 0x5fdfffff - 0x200000 - vramsize + 1;	//end pa - tee size - fb buffer size
#endif
	return fb_base;
}
...
#ifdef CONFIG_FPGA_EARLY_PORTING
        _mtkfb_internal_test(fbdev->fb_va_base, MTK_FB_XRES, MTK_FB_YRES);   //lk is ready , disable internal test
#endif
