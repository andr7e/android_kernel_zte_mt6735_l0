...
DPI_STATUS ddp_dpi_ConfigPclk(cmdqRecHandle cmdq, unsigned int clk_req, DPI_POLARITY polarity)
{
    ...
    */	
	...
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "DISPSYS_TIMING1: 0x%x\n", INREG32(DISPSYS_TIMING1));
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "DISPSYS_TIMING2: 0x%x\n", permission);
	...
	if((permission & 0xC0000000) == 0)
	{
		...
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI", "DISPSYS_TIMING3: 0x%x, DISPSYS_TIMING1: 0x%x\n", INREG32(DISPSYS_TIMING3), INREG32(DISPSYS_TIMING1));
	}
