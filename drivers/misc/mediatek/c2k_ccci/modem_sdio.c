	...
	while (fifo_total_count > 0){
		...
		do{
			...
			if (ccmni_port->index == CCMNI_AP_LOOPBACK_CH-1){
				...
				LOGPRT(LOG_INFO,  "%s %d: port%d sending to md(len %d).\n", __func__, __LINE__,ccmni_port->index, write_len);
				modem_sdio_write(modem, SDIO_WRITE_ADDR, modem->trans_buffer, write_len);
			}
			left -= todo;
		}while(left);
...
int sdio_modem_ccmni_send_pkt(int md_id, int tx_ch, void *data)
{
	...
	if(data_len > (FIFO_SIZE - chars_in_fifo)){
		LOGPRT(LOG_ERR,  "%s %d FIFO size is not enough!(%d)\n", __func__,__LINE__, chars_in_fifo);
		skb_pull(skb, sizeof(struct sdio_msg_head));
		return CCMNI_ERR_TX_BUSY;
	}
...
get_one:
	...
	//LOGPRT(LOG_INFO, "reading on port%d %p size=%d offset=%d read_curr=%d\n", port->index, 
		//packet, packet->size, packet->offset, read_curr); 
