...
#include "ov5670mipi_syx_Sensor.h"
...
static int RG_Ratio_Typical = 0x137;//0x280;   // 315
static int BG_Ratio_Typical = 0x146;	//291
...
//static kal_uint8 mirror_flip = IMAGE_H_MIRROR; //xuqian add
...
//	write_cmos_sensor(0x3655, 0x20); 
	...
	set_mirror_flip(imgsensor.mirror);//xuqian add 20140929
#ifdef OV5670_OTP
	printk("pxs_ov5670_otp ++++++++++++++++  %s \n", __func__); //pei_add	
	mdelay(40);
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				sensor_init();
				if(current_otp.module_integrator_id == 0x10) {
					LOG_INF("module_integrator_id 0x%x == 0x10\n", current_otp.module_integrator_id);
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				LOG_INF("ov5670 syx found\n");
				return ERROR_NONE;
			}	
			LOG_INF("Read ov5670 sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
UINT32 OV5670_MIPI_SYX_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
