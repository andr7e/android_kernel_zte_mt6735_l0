...
static imgsensor_info_struct imgsensor_info = { 
  ...
  .mipi_settle_delay_mode = 1,//0,MIPI_SETTLEDELAY_AUTO; 1,MIPI_SETTLEDELAY_MANNUAL
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb, //SENSOR_OUTPUT_FORMAT_RAW_Gr,//SENSOR_OUTPUT_FORMAT_RAW_B,//sensor output first pixel color
	.mclk = 24,//mclk value, suggest 24 or 26 for 24Mhz or 26Mhz
	.mipi_lane_num = SENSOR_MIPI_4_LANE,//mipi lane num
	.i2c_addr_table = {0x6c, 0x20, 0x42, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};


static imgsensor_struct imgsensor = {
	.mirror = IMAGE_HV_MIRROR, //IMAGE_HV_MIRROR,//IMAGE_NORMAL,				//mirrorflip information
...
//  ;;
	...
	write_cmos_sensor(0x3012, 0x20);//0x6c);
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	if(addr != 0) {
		...
		printk("otp checksum2=0x%x, 0x%x",checksum2, (*otp_ptr).checksum);
	}
	else {
		for(i=0;i<240;i++) {
			(* otp_ptr).lenc[i]=0;
...
{
	int RG_Ratio_Typical = 0x126, BG_Ratio_Typical = 0x122;
