...
#include "ov13850mipiraw_sjc_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "OV13850_camera_sensor_sjc"
...
//sensor otp
extern void otp_cali_sjc(unsigned char writeid);
extern int read_otp_vendor_sjc(unsigned char writeid);
...
static imgsensor_struct imgsensor = {
    .mirror = IMAGE_V_MIRROR,//IMAGE_H_MIRROR,                //mirrorflip information
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
            	sensor_init();
								otp_vendor = read_otp_vendor_sjc(imgsensor.i2c_write_id);
								printk("otp_vendor=0x%x\n", otp_vendor);
								if(0x10 != otp_vendor) {
...
static kal_uint32 open(void)
{
    ...
    sensor_init();
	  //for OTP
	  otp_cali_sjc(imgsensor.i2c_write_id);
	  write_cmos_sensor(0x0100, 0x00);
...
UINT32 OV13850_MIPI_RAW_sjc_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
