...
#define MMDVFS_ENABLE	((IS_ENABLED(CONFIG_ARCH_MT6735) || IS_ENABLED(CONFIG_ARCH_MT6735M)) && !IS_ENABLED(CONFIG_FPGA_EARLY_PORTING))
...
#define MMDVFS_DISPLAY_SIZE_HD	(1280 * 736)
...
static mmdvfs_voltage_enum mmdvfs_query(MTK_SMI_BWC_SCEN scenario, MTK_MMDVFS_CMD *cmd)
{
	...
	switch (scenario) {
#if IS_ENABLED(CONFIG_ARCH_MT6735M)	/* D2 ISP >= 6M HIGH */
...
void mmdvfs_notify_scenario_enter(MTK_SMI_BWC_SCEN scen)
{
	...
	/* ISP ON = high */
	switch (scen) {
#if IS_ENABLED(CONFIG_ARCH_MT6735M)	/* d2 sensor > 6M */
		case SMI_BWC_SCEN_VR:
			mmdvfs_set_step(scen, mmdvfs_query(scen, NULL));
			break;
#else	/* default VR high */
		case SMI_BWC_SCEN_VR:
#endif /* IS_ENABLED(CONFIG_ARCH_MT6735) */
		case SMI_BWC_SCEN_VR_SLOW:
