...
extern bool gCmdqPrintDumpError0_Flag;
#endif
...
#define CMDQ_LOG(string, args...) \
	...
	cmdq_core_save_dump_error0("[CMDQ]"string, ##args); \
	}
...
#define CMDQ_ERR(string, args...) \
	...
	cmdq_core_save_dump_error0("[CMDQ][ERR]"string, ##args); \
	}
...
#define CMDQ_AEE(tag, string, args...) \
	...
	cmdq_core_save_dump_error0("[CMDQ][AEE]"string, ##args); \
 ...
 * Set a given value to CMDQ secure HW thread's THR executed count to secShareMemory
 * Parameter:
 *    string: [IN] the desired string, which should be save in a preallocated buffer
 * Return:
 *     = 0 for successfully, < 0 for access failed
 */
