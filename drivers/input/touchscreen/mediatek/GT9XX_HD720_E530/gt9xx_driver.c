...
//************* For GT9XXF End **********************//
#if GTP_GESTURE_WAKEUP
static int enable_wakeup_gesture = 0;
struct kobject *gt9xx_debug_kobj = NULL;

static ssize_t gt9xx_wake_gesture_show(struct device *dev,
...
static ssize_t gt9xx_wake_gesture_store(struct device *dev,
...
static ssize_t gt9xx_configid_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ret = -1;
	u32 version_info = 0;
	...
	if (ret < 0)
		...
		version_info = (version_info << 8) | 	cfg_info_group0[0];
		
	return snprintf(buf, PAGE_SIZE, "%06x\n", version_info);
}

static ssize_t gt9xx_configid_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}
...
static DEVICE_ATTR(wake_gesture, (S_IRUGO | S_IWUGO),
			gt9xx_wake_gesture_show,
			gt9xx_wake_gesture_store);

static DEVICE_ATTR(configid, (S_IRUGO | S_IWUGO),
			gt9xx_configid_show,
			gt9xx_configid_store);
#endif
void gt9xx_sysfs(void){
#if GTP_GESTURE_WAKEUP
			sysfs_create_file(&tpd->dev->dev.kobj,
				&dev_attr_wake_gesture.attr);
			sysfs_create_file(&tpd->dev->dev.kobj,
				&dev_attr_configid.attr);
#endif
}
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            if (ret > 0)
            {
                    ...
                    )
                {
                    ...
                    if(doze_buf[2] == 'o') key = KEY_O;
                    if(doze_buf[2] == 'w') key = KEY_W;
                    if(doze_buf[2] == 'm') key = KEY_M;
                    if(doze_buf[2] == 'c') key = KEY_C;
/*                    input_report_key(tpd->dev, KEY_POWER, 1);
                ...
                if(key) {
                    ...
                    input_sync(tpd->dev);
                    input_report_key(tpd->dev, key, 0);
                    input_sync(tpd->dev);
                  }
            }
			mutex_unlock(&i2c_access);
        ...
        key_value = point_data[3 + 8 * touch_num];
				...
				GTP_DEBUG("key_value:%02x.", key_value);
